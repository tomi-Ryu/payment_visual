...
def get_Rakuten_Billing_Statement_Csv():

  LOGIN_URL = "https://www.rakuten-card.co.jp/e-navi/auth/login.xhtml"
  # "tabNo=" の直後に数字を入れて、参照月を指定。0:来月分支払い(今月利用分), 1: 今月支払い・・・
  RAKUTEN_BILLING_STATEMENT_BASE_URL = "https://www.rakuten-card.co.jp/e-navi/members/statement/index.xhtml?tabNo="
  # 絶対パスでないと動作しない
  ...
  def get_rakuten_billing_statement(month_span):
    for m_No in range(month_span):
      Url_curMonth = RAKUTEN_BILLING_STATEMENT_BASE_URL + str(m_No)
